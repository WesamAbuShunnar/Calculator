...
public class Controller   {
	...
	Controller(View v1,Model m1 ){
		...
		v.buttons[17].addActionListener(handler); // %
		v.buttons[18].addActionListener(handler); // (-)
		v.buttons[19].addActionListener(handler); // (.)
	...
	public void actionPerformed(ActionEvent event){
		...
		else if(event.getSource() ==v.buttons[17]){
			// mod
			res = "";
			op = "%";
		}
		...
		else if(event.getSource() == v.buttons[18]){
			// negative
			if (s.equals("0"))
				res = "-";
			else
		...
		else if(event.getSource() == v.buttons[19]){
